...
- $\textcolor{blue}{x}_{p} \in \{0,1\}$: takes value $1$ if and only if polyomino $p\in P$ is used
...
\begin{align}
\sum_{p \in P|c \in p}\textcolor{blue}{x}_{p}\le 1 \quad \forall c \in C \\
\end{align}
...
$$
\textcolor{blue}{x}_{p}+\textcolor{blue}{x}_{q} \le 1 \quad \forall (p,q) \in P \times P, \; N(p)\cap q = \emptyset
$$
...
If the set of polyominoes is the L $6\times 3$ octominoes, $|K|=1$, and constraints $(1)$ 
are adjusted as follows to enforce $4$ polynominoes:

$$
\sum_{p \in P_k}\textcolor{blue}{x}_{p} = 4\quad \forall k\in K
$$
